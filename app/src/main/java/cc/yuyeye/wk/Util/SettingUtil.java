package cc.yuyeye.wk.Util;

public class SettingUtil {
	public final static String DEV_UPDATE_KEY = "dev_update_key";
	public final static String SHOW_NETTYPE_KEY = "show_nettype_key";
    public final static String ID_KEY = "id_edittext_key";
    public final static String QQ_KEY = "qq_edittext_key";
    
    public final static String LOG_KEY = "log_switch_key";
    public final static String ENTER_SEND_KEY = "enterSend_switch_key";
    public final static String SAVE_MSG_KEY = "saveMsg_switch_key";
    public final static String MSG_TYPE_KEY = "msg_type_key";
    public final static String CONTACTS_TIPS = "contacts_tips";
    public final static String UPDATE_SOURCE = "update_source";
    public final static String ABOUT_KEY = "about_key";
    public final static String WK_TIMELINE_KEY = "wk_timeline_key";
}
